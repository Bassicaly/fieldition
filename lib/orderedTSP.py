...
class OTSPstate:
    ...
    def agents_new_time(self, agent):
    ...
    def split(self, num):
        ...
        for agent in range(self.nagents):
            newtime = self.agents_new_time(agent)
    ...
    def calc_times(self):
        ...
        # These could be pre-allocated if agents_new_time didn't use negativ index
        ...
        for i in range(1, nvisits):
            agent = self.visit2agent[i]

            t = self.agents_new_time(agent)
...
def get_visits(dists, order, nagents):
    """
